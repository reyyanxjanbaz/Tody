...
export function isFullyDecayed(task: Task): boolean {
  ...
  // Normalize both ends to start-of-day so this matches getDaysOverdue exactly.
  // (Previously this used raw Date.now() ms division, which disagreed with the
  // displayed "N days ago" count by up to a day at the archive boundary.)
  const now = startOfDay().getTime();
  const overdueStart = startOfDay(new Date(task.overdueStartDate || task.deadline)).getTime();
