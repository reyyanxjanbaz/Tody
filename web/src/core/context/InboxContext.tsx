...
import { loadTombstoneIds, recordTombstone } from '../utils/tombstones';
...
export function InboxProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Ids deleted on this device whose deletion may not have propagated.
        const deletedIds = await loadTombstoneIds('inbox');

        // Fetch from DB
        const dbItems = await fetchDbInbox();

        // Merge: DB wins on conflicts, dropping tombstoned ids.
        if (dbItems.length > 0) {
          setInboxTasks(prev => {
            ...
            return Array.from(merged.values()).filter(t => !deletedIds.has(t.id));
          });
        }

        // Push local-only items to Supabase — never re-insert a tombstoned id.
        const localItems = tasksRef.current.filter(t => !deletedIds.has(t.id));
  ...
  const removeInboxTask = useCallback((id: string) => {
    recordTombstone('inbox', id).catch(() => {}); // guard against sync resurrection
