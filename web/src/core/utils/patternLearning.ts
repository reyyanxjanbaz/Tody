...
// TaskPattern carries no timestamp, so true age-based expiry would need a
// schema field (out of scope). Instead we bound the store so it can't grow
// without limit: keep at most MAX_PATTERNS, dropping the least-evidenced ones
// (lowest sample size) first — those are the weakest, most likely stale.
...
async function savePatterns(patterns: TaskPattern[]): Promise<void> {
  const bounded = patterns.length > MAX_PATTERNS
    ? [...patterns].sort((a, b) => b.sampleSize - a.sampleSize).slice(0, MAX_PATTERNS)
    : patterns;
  await AsyncStorage.setItem(KEYS.TASK_PATTERNS, JSON.stringify(bounded));
  // Fire-and-forget cloud sync after every local save
  pushPatternsToCloud(bounded).catch(() => {});
}
