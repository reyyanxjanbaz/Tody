...
export function calculateUserStats(tasks: Task[]): UserStats {
  ...
  // Canonical formula (must stay identical to server get_reality_score in
  // server/routers/profile.py): 100 - |estimated - actual| / ESTIMATED * 100,
  // clamped >= 0, default 0 when there's no estimated time. The denominator is
  // ESTIMATED (what the user predicted) so the score answers "how close were
  // your predictions", not "how close relative to reality".
  const realityScore = totalEstimatedMinutes > 0
    ? Math.max(0, Math.round(100 - (Math.abs(totalEstimatedMinutes - totalActualMinutes) / totalEstimatedMinutes * 100)))
    : 0;
